...
if [[ "$(os.platform)" == "linux" && ! -e /tmp/user-scaling.timer ]]; then
  ...
  if [[ -e "$zedConfig" && -n "$zedMono" ]]; then
    ...
    currentZedAgentBufferFontSize="$(grep '"agent_buffer_font_size"' "$zedConfig" | cut -d: -f2 | sed -e 's|"||g' -e 's|,||g' -e 's|^[[:space:]]*||g' -e 's|[[:space:]]*$||g')"
    ...
    wantedZedAgentBufferFontSize="$zedAgentMono"
    ...
    # Update Zed agent buffer font if different.
    if [[ -n "$currentZedAgentBufferFontSize" && "$currentZedAgentBufferFontSize" != "$wantedZedAgentBufferFontSize" ]]; then
      log.info "Setting Zed agent buffer font to: \"$currentZedBufferFontName $wantedZedAgentBufferFontSize\""
      currentZedAgentBufferFontString="\"agent_buffer_font_size\": $currentZedAgentBufferFontSize,"
      wantedZedAgentBufferFontString="\"agent_buffer_font_size\": $wantedZedAgentBufferFontSize,"
      sed -i "s|$currentZedAgentBufferFontString|$wantedZedAgentBufferFontString|g" "$zedConfig"
    fi
  ...
  nvimConfig="$HOME/.config/nvim/lua/appearance.lua"
